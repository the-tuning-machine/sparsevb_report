"""
Tune Stability Selection hyperparameters
=========================================
Grid search over subsample_frac, gamma_threshold, and freq_threshold
across 6 problem settings. Reports the best configurations by F1 score.
"""
...
print(f"Stability Selection tuning")
...
print("\n" + "=" * 90)
print("TOP 10 STABILITY CONFIGURATIONS")
print("=" * 90)
print(f"{'Rank':>4} | {'subsamp':>7} | {'gamma_th':>8} | {'freq_th':>7} | "
      f"{'F1':>5} | {'Prec':>5} | {'Rec':>5} | {'TP':>5} | {'FP':>5} | {'FN':>5}")
print("-" * 90)
for i, r in enumerate(param_scores[:10]):
    p = r["params"]
    print(f"{i+1:4d} | {p['subsample_frac']:7.1f} | {p['gamma_threshold']:8.1f} | "
          f"{p['freq_threshold']:7.1f} | {r['f1']:5.3f} | {r['precision']:5.3f} | "
          f"{r['recall']:5.3f} | {r['tp']:5.1f} | {r['fp']:5.1f} | {r['fn']:5.1f}")

print(f"\nBEST: {param_scores[0]['params']}  (F1={param_scores[0]['f1']:.3f})")
